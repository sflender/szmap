...
int main(int argc, char *argv[]){
	...
	cosmo cosm_model(H0, Omega_M, Omega_b, Omega_k, w_DE); // construct the cosm. model (needed for ang diam distance)
   	...
   	int completed = 0;
	for (int j=rank+1; j<nsel; j+=nranks){ // distribute the halos evenly over all MPI ranks
		int i = sel[j];
		...
		for (int p=0; p<npix_in_disc; p++){ // loop over all pixels in the disc
			...
			//for (long int q = pix_mult*listpix[p]; q<pix_mult*(listpix[p]+1); q++){ // loop over all subpixels inside the main pixel
			int q = listpix[p];
				vec3 vec_to_pix = HPB_subpix.pix2vec(q);
				...
				//if(rank==0) cout<<i<<" "<<p<<" "<<npix_in_disc<<" "<<q<<" "<<listpix[p]<<" "<<theta_max*rad2arcmin <<" "<< d_halo_pixel/R200<<" "<< M200<<" "<< R200<<" "<< redshift[i]<<" "<< rhocrit<<" "<< Omega_b/Omega_M<<endl;

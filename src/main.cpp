...
int main(){
	...
	bool convert_map_to_ring = cfg.getValueOfKey<"convert_map_to_ring">;
    ...
    //---other
    float max_radius = cfg.getValueOfKey<float>("max_radius"); // where to truncate the profile (in units of R200c)
	...
	int nside_subpix = pow(2,order_subpix);
	int npix = 12*pow(nside,2);
	int npix_subpix = 12*pow(nside_subpix,2);

	Healpix_Ordering_Scheme scheme = NEST;

	Healpix_Base HPB = Healpix_Base(order,scheme);
	Healpix_Base2 HPB_subpix = Healpix_Base2(order_subpix,scheme);

	cout<<"low res npix = "<<npix_lr<<endl;
	cout<<"high res npix = "<<npix_hr<<endl;
	cout<<"pixel multiplier = "<<pix_mult;
	...
	cosmo cosm_model(H0, Omega_M, Omega_b, Omega_k, wt); // construct the cosm. model (needed for ang diam distance)
	...
	for (int j=rank; j<nsel; j+=nranks){ // distribute the halos evenly over all MPI ranks
        ...
        float theta_max = max_radius * theta200;
        
	    vector<int> listpix; //list of pixel indices in disc generated by query_disc

	    HPB.query_disc(ptg, theta_max, listpix);
		int npix_in_disc = listpix.size();

		for (int p=0; p<npix_in_disc; p++){
			vec3 vec_to_pix = HPmap->pix2vec(listpix[p]);
			float alpha = acos(dotprod(vec_to_cluster,vec_to_pix)); // angular separation in rad
			float d_halo_pixel = alpha * d_obs_halo; // physical distance from halo center to pixel p (Mpc)
			float ysz = return_ysz_Battaglia(max_radius, d_halo_pixel/R200, M200, R200, redshift[i], rhocrit, Omega_b/Omega_M);
			float tsz = ysz * tsz_norm;
			local_pixel_array[listpix[p]] += tsz; 
		}
		
		listpix.clear();
	
		if (completed%100==0 and rank==0) cout<<"rank "<<rank<<" completed tSZ profile "<<completed<<" of "<<nsel/nranks<<"\n";
		completed++;
	}
	...
	if (rank==0 and write_output_file==1){
		...
		HPmap->SetNside(nside,Healpix_Ordering_Scheme);

...
int main(int argc, char *argv[]){
    ...
    int profiletype = cfg.getValueOfKey<int>("profiletype");
    ...
    float beta = cfg.getValueOfKey<float>("beta");
	...
	for (int j=rank; j<nsel; j+=nranks){ // distribute the halos evenly over all MPI ranks
        ...
        //printf("pt %d beta %f", profiletype, beta);
		for (int p=0; p<npix_in_disc; p++){ // loop over all pixels in the disc
			...
			for (long int q = (long int)pix_mult* (long int)(*listpix)[p]; q < (long int)pix_mult*( (long int)(*listpix)[p]+1); q++){ // loop over all subpixels inside the main pixel
                ...
                if(profiletype == 1){
                    ysz = return_ysz_Battaglia(max_int_radius, d_halo_pixel/R200, M200, R200, redshift[i], rhocrit, Omega_b/Omega_M);
                }
                else if(profiletype == 2){
                    //float ysz = return_ysz_Beta_WB(max_int_radius, alpha, M200, redshift[i], h, Omega_M, Omega_Mz, Delta_vir, d_obs_halo, rhocrit);
                    ysz = return_ysz_Beta_Birkinshaw(beta, alpha, M200, redshift[i], h, Omega_M, Omega_Mz, Delta_vir, d_obs_halo, rhocrit);
                     //printf("pt %d ysz %.e\n", profiletype, ysz); 
                }
				float tsz = ysz * tsz_norm;
                //printf("alpha %f tsz %f\n", alpha, tsz);
